...
Note also that neighbors/connectivities need to be computed for each sample individually. If you have multiple samples in a single `AnnData` object, you can use the `slide_key` parameter to compute connectivities for each sample separately.
...
# Lets also save the "high granularity" cell type assignments to the `adata.obs` table by simply takine the argmax over the scores for each cell ID.
{% highlight python linenos %}
...
Now lets plot the elbow plot using the meanchange from the ranking dataframe above:
...
We were able to replicate the results shown in the tutorial from 10X using python and adding the cell segmentation. This gives more freedom to explore the data and the biology behind it. 

Next steps: 
Liana+ (https://liana-py.readthedocs.io/en/latest/) for LR and CCC, decoupler, etc